...
describe('analysis', function() {
  ...
  describe('of term uses', function() {
    it('produces an object', function() {
      expect(analyze({content: ['test']}).uses).to.be.an('object');
    });

    it('reports term uses', function() {
      expect(analyze({content: [{use: 'Agreement'}]}).uses)
        .to.eql({Agreement: [['content', 0]]});
    });
  ...
  describe('of child headings', function() {
    it('produces an object', function() {
      expect(analyze({content: ['test']}).headings).to.be.an('object');
    });
    ...
    it('reports nested headings', function() {
      ...
      ).to.eql({Indemnity: [['content', 0, 'form', 'content', 0]]});
    });
    ...
    it('reports multiple paths for >1', function() {
      expect(
        analyze({
          content: [
            {heading: 'Indemnity', form: {content:['test']}},
            {heading: 'Indemnity', form: {content:['test']}}
          ]
        }).headings
      ).to.eql({Indemnity: [['content', 0], ['content', 1]]});
    });
  });

  describe('of references', function() {
    it('produces an object', function() {
      expect(analyze({content: ['test']}).references)
        .to.be.an('object');
    });

    it('reports references made', function() {
      expect(analyze({content: [{reference: 'Indemnity'}]}).references)
        .to.eql({Indemnity: [['content', 0]]});
    });
    ...
    it('reports nested references', function() {
      expect(
        analyze({
          content: [{form: {content: [{reference: 'Indemnity'}]}}]
        }).references
      ).to.eql({Indemnity: [['content', 0, 'form', 'content', 0]]});
    });
    ...
    it('reports multiple references', function() {
      expect(
        analyze({
          content: [{reference: 'Indemnity'}, {reference: 'Indemnity'}]
        }).references
      ).to.eql({Indemnity: [['content', 0], ['content', 1]]});
    });
  });
  ...
  describe('of blanks', function() {
    it('produces an object', function() {
      expect(analyze({content: ['test']}).blanks).to.be.an('object');
    });

    it('reports blanks made', function() {
      expect(analyze({content: [{blank: 'Seller'}]}).blanks)
    ...
    it('reports nested blanks', function() {
      expect(
        analyze({content: [{form: {content: [{blank: 'Seller'}]}}]})
          .blanks
      ).to.eql({Seller: [['content', 0, 'form', 'content', 0]]});
    });

    it('reports multiple blanks', function() {
      expect(
        analyze({content: [{blank: 'Date'}, {blank: 'Date'}]}).blanks
      ).to.eql({Date: [['content', 0], ['content', 1]]});
    });
  });
